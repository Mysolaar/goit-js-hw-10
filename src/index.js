...
const searchCountriesList = document.querySelector('.country-list');
const searchCountriesLinfo = document.querySelector('.country-info');

const clearCountryElements = () => {
  searchCountriesList.innerHTML = '';
  searchCountriesLinfo.innerHTML = '';
};
...
const makeCountryHtml = debounce(event => {
  ...
  fetchCountries(event.target.value)
    .then(data => {
      if (data.length > 10) {
        ...
        clearCountryElements();
        searchCountriesList.append(...createCountryListItems(data));
      } else {
        const [header, list] = createCountryInfo(data[0]);
        searchCountriesLinfo.innerHTML = '';
        searchCountriesLinfo.append(header, list);
      }
    })
    .catch(error => {
...
searchInput.addEventListener('input', makeCountryHtml);